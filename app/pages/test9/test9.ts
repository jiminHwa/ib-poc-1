...
@Component({
    templateUrl: 'build/pages/test9/test9.html'
})
export class Test9Page {

    bubbleOn: boolean = true;
    shadowOn: boolean = false;

    indexOfBubbleChild: {shadow: number, circle: number, text: number};
    ...
    constructor(private nav: NavController, private el: ElementRef) {
        ...
        if (this.shadowOn) {
            this.indexOfBubbleChild = {shadow: 0, circle: 1, text: 2};
        }
        else {
            this.indexOfBubbleChild = {shadow: -1, circle: 0, text: 1};
        }
    }
    ...
    ionViewWillEnter() {
        ...
        if (self.indexOfSelectedBubble >= 0) {
            self.deselectBubble(300, function() {
                self.stageInteractionBlocked = false;
                self.bgInteractionBlocked = false;
        ...
        function animate() {
            ...
            if (self.bubbleOn) {
                ...
                for (let i = 0; i < bubbles.length; i++) {
                    let circle = bubbles[i].getChildAt(self.indexOfBubbleChild.circle);
    ...
    createAPage(index: number) {
        ...
        for (let i = 0; i < this.numOfBubbles; i++) {
            ...
            if (self.shadowOn) {
                ...
                bubble.addChildAt(shadow, self.indexOfBubbleChild.shadow);
            }
            ...
            bubble.addChildAt(circle, self.indexOfBubbleChild.circle);
            ...
            if (i === Math.floor(self.numOfBubbles / 2)) {
                let newText: any;
                newText = self.getText('' + (index - 1));
                bubble.addChildAt(newText, self.indexOfBubbleChild.text);
            }
    ...
    changePageNumber(page: any, num: number) {
        let bubble = page.bubbles[Math.floor(this.numOfBubbles / 2)];
        let oldText = bubble.getChildAt(this.indexOfBubbleChild.text);
        bubble.removeChildAt(this.indexOfBubbleChild.text);
        oldText.destroy(false);
        let newText = this.getText('' + num);
        bubble.addChildAt(newText, this.indexOfBubbleChild.text);
    }
    ...
    private onPageSlideingSucceed(tweens: any[], page: any, nextPage: any, indexOfNextPage: number, distX: number) {
        ...
        for (let i = 0; i < this.pageWindow; i++) {
            ...
            if (distX > 0 && i === this.pageWindow - 1) {
                newPosX = ((0 - this.indexOfPageOnScreen) * this.canvasSize.w);
            }
            else if (distX < 0 && i === 0) {
                newPosX = ((this.pageWindow - 1 - this.indexOfPageOnScreen) * this.canvasSize.w);
            }
            else {
                newPosX = ((i - this.indexOfPageOnScreen) * this.canvasSize.w);
        ...
        if (distX > 0) {
            ...
            let pageTemp = this.pages[this.pageWindow - 1].page;
            let bubblesTemp = this.pages[this.pageWindow - 1].bubbles;
            ...
            this.pages[0].page = pageTemp;
            this.pages[0].bubbles = bubblesTemp;

            // this.createAPage(0, this.pageNumber - this.indexOfPageOnScreen);
            // for(let i = 0 ; i < this.pageWindow ; i ++) {
            //     this.changePageNumber(this.pages[i], i + this.pageNumber - this.indexOfPageOnScreen);
            // }
            this.changePageNumber(this.pages[0], this.pageNumber - this.indexOfPageOnScreen);
        }
        else if (distX < 0) {
            this.pageNumber++;
            ...
            let pageTemp = this.pages[0].page;
            let bubblesTemp = this.pages[0].bubbles;
            ...
            this.pages[this.pageWindow - 1].page = pageTemp;
            this.pages[this.pageWindow - 1].bubbles = bubblesTemp;
            // this.createAPage(this.pageWindow - 1, this.pageNumber + this.indexOfPageOnScreen);
            this.changePageNumber(this.pages[this.pageWindow - 1], this.pageNumber + this.indexOfPageOnScreen);
    ...
    private deselectBubble(duration: number = 1000, onComplete: any = undefined) {
        if (this.indexOfSelectedBubble >= 0) {
            let tweens: any[] = [];
            let bubbles = this.pages[this.indexOfPageOnScreen].bubbles;
